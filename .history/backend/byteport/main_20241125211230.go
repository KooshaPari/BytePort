...
func main() {
	...
	// Decrypt the AppID to verify persistence
	decryptedAppID, err := lib.DecryptSecret(temp.AppID)
	if err != nil {
		fmt.Printf("Error decrypting AppID: %v\n", err)
		os.Exit(1)
	}

	// Print the decrypted AppID to confirm persistence
	fmt.Printf("Decrypted AppID: %s\n", decryptedAppID)
	r := setupRouter()
