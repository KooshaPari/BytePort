...
func LinkHandler(c *gin.Context) {
	...
	fmt.Println(req)
	user.AwsCreds, _ = lib.EncryptSecret(req.AwsCreds, os.Getenv("ENCRYPTION_KEY"))
	user.OpenAICreds, _ = lib.EncryptSecret(req.OpenAICreds, os.Getenv("ENCRYPTION_KEY"))
	user.Portfolio, _ =lib.EncryptSecret(req.Portfolio, os.Getenv("ENCRYPTION_KEY"))
	user.Git, _ =lib.EncryptSecret(req.AwsCreds, os.Getenv("ENCRYPTION_KEY"))
	models.DB.Save(&user)
