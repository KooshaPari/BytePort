...
func LinkHandler(c *gin.Context) {
	...
	encryptedAuthMethod, err := lib.EncryptSecret(req.Git.AuthMethod, os.Getenv("ENCRYPTION_KEY"))
	if(err!=nil){
		log.Fatal(err)
	}
	encryptedAuthKey, err := lib.EncryptSecret(req.Git.AuthKey, os.Getenv("ENCRYPTION_KEY"))
	if(err!=nil){
		log.Fatal(err)
	}
