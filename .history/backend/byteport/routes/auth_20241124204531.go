...
func LinkHandler(c *gin.Context) {
	...
	lib.ValidateAWSCredentials(req.AwsCreds.AccessKeyID,req.AwsCreds.SecretAccessKey)
	lib.ValidateGitRepo(req.Git.AuthKey, req.Git.AuthMethod, req.Git.RepoURL, req.Git.TargetDirectory
	)
	lib.ValidateOpenAICredentials()
	lib.ValidatePortfolioAPI()

	// Encrypt sensitive fields
	key,err := lib.GetDecodedEncryptionKey()
	encryptionKey := string(key)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to get key", "details": err.Error()})
		return
	}

	encryptedPortfolioAPIKey, err := lib.EncryptSecret(req.Portfolio.APIKey, encryptionKey)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt Portfolio API Key", "details": err.Error()})
		return
	}

	encryptedRepoURL, err := lib.EncryptSecret(req.Git.RepoURL, encryptionKey)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt Git Repo URL", "details": err.Error()})
		return
	}

	encryptedAuthMethod, err := lib.EncryptSecret(req.Git.AuthMethod, encryptionKey)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt Git Auth Method", "details": err.Error()})
		return
	}

	encryptedAuthKey, err := lib.EncryptSecret(req.Git.AuthKey, encryptionKey)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt Git Auth Key", "details": err.Error()})
		return
	}

	encryptedTargetDirectory, err := lib.EncryptSecret(req.Git.TargetDirectory, encryptionKey)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt Git Target Directory", "details": err.Error()})
		return
	}

	encryptedAccessKeyID, err := lib.EncryptSecret(req.AwsCreds.AccessKeyID, encryptionKey)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt AWS Access Key ID", "details": err.Error()})
		return
	}

	encryptedSecretAccessKey, err := lib.EncryptSecret(req.AwsCreds.SecretAccessKey, encryptionKey)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt AWS Secret Access Key", "details": err.Error()})
		return
	}

	encryptedApiKey, err := lib.EncryptSecret(req.OpenAICreds.APIKey, encryptionKey)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to encrypt OpenAI API Key", "details": err.Error()})
		return
	}

	encryptedPortfolioURL, err := lib.EncryptSecret(req.Portfolio.RootEndpoint, encryptionKey)
	if err != nil {
