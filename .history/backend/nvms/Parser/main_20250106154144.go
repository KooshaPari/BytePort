...
func init() {
	spinhttp.Handle(func(w http.ResponseWriter, r *http.Request) {
		...
		nvms, err = parseNVMSConfig(nvmsString)
		if err != nil {
			http.Error(w, fmt.Sprintf("Failed to parse NVMS config: %v", err), http.StatusBadRequest)
			return
		}
		// return NVMS Object
		nvmsBytes, err := json.Marshal(nvms)
		if err != nil {
			http.Error(w, fmt.Sprintf("Failed to marshal NVMS object: %v", err), http.StatusInternalServerError)
			return
		}
