...
import (
	...
	"log"
	"path/filepath"
	imageBuilder "nvms/lib/awspin/ec2/imageBuilder"
	aws "nvms/lib/awspin"
...
func TerminateS3(resource models.AWSResource, AccessKey string, SecretKey string)(error){
 ...
 func TerminateTargetGroup(resource models.AWSResource, AccessKey string, SecretKey string)(error){c, err := awsnet.NewALB(aws.Config{
   ...
   err = c.CreateRepository(context.Background(), repoName)
   if err != nil {
	   fmt.Println(err)
	   return "",err
   }
...
func CreateImgPipeline(AccessKey string, SecretKey string, projectName string, repoName string) (string, error) {
	c, err := imagebuilder.NewCodePipeline(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
   ...
   pipelineName := projectName+"-byteport"
   err = c.CreatePipeline(context.Background(), pipelineName, repoName)
   if err != nil {
	   fmt.Println(err)
	   return "",err
   }
