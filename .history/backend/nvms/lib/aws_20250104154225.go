...
func CreateImgPipeline(AccessKey string, SecretKey string, projectName string, repoName string) (CreatePipelineResponse, error) {
	c, err := ec2.NewEC2(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
   ...
   piplelineName := projectName+"-byteport-" + uuid.New().String()
