...
func CreateInfrastructureConfiguration(AccessKey string, SecretKey string, projectName string, instProfARN string) (string, error) {
	c, err := ec2.NewEC2(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
   ...
   infra, err := c.CreateInfrastructureConfiguration(context.Background(), infraName, instProfARN)
   if err != nil {
	   fmt.Println(err)
	   return "",err
   }
...
func ExecuteImgPipeline(AccessKey string, SecretKey string, projectName string, instProfArn string )(ec2.ExecuteImgPipelineResponse, error){
	c, err := ec2.NewEC2(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
   ...
   pipeline, err := c.ExecuteImgPipeline(context.Background(), pipelineArn, instProfArn)
   if err != nil {
	   fmt.Println(err)
	   return ec2.ExecuteImgPipelineResponse{},err
   }
...
func CreateInstanceProfile(AccessKey string, SecretKey string, projectName string )(ec2.CreateInstanceProfileResponse, error){
	...
	if err != nil {
		fmt.Println(err)
		return ec2.CreateInstanceProfileResponse{},err
   }
   instProfName := projectName+"-byteport-" + uuid.New().String()
   instProf, err := c.CreateinstProfstructureConfiguration(context.Background(), instProfName, instProfARN)
   if err != nil {
	   fmt.Println(err)
	   return "",err
   }
   fmt.Println("instProfstructure created successfully.")
   return  instProf,nil  
