...
func CreateContainerRecipe(AccessKey string, SecretKey string, bucket S3DeploymentInfo, service models.Service, fileMap []string, infraConfig string) (*ec2.CreateContainerRecipeResponse, error) {
	c, err := ec2.NewEC2(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
   ...
   recipeName := service.Name+"-byteport-" + uuid.New().String()
   recipe, err := c.CreateContainerRecipe(context.Background(), components,recipeName, bucket, service, fileMap, infraConfig)
   if err != nil {
	   fmt.Println(err)
	   return nil,err
   }
