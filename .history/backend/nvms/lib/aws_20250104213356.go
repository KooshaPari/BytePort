...
func CreateImageComponents(AccessKey string, SecretKey string, workdir string, name string,s3Info S3DeploymentInfo, service models.Service, files []string) ([]models.ImageComponent, error) {
   ...
   if err != nil {
	...
	// image data is an object representation of the yaml file with yaml tags, unmarshal to a string in yaml format
	if err := yaml.Unmarshal([]byte(imgData), &data); err != nil {
			fmt.Println(err)
			return []models.ImageComponent{},err
		}
   ...
   for _, comp := range []string{"base", "dependencies", "app"} {
	   compName := name+"-"+comp
	   comp, err := c.CreateImageComponent(context.Background(), compName,"1.0.0", "Linux", data)
	   if err != nil {
		   fmt.Println(err)
		   return []models.ImageComponent{},err
	   }
