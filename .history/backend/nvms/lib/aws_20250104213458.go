...
func TerminateS3(resource models.AWSResource, AccessKey string, SecretKey string)(error){
 ...
 func TerminateTargetGroup(resource models.AWSResource, AccessKey string, SecretKey string)(error){c, err := awsnet.NewALB(aws.Config{
   ...
   func CreateECRRepo(AccessKey string, SecretKey string, projectName string) (*ec2.CreateRepositoryResponse, error) {
	c, err := ec2.NewEC2(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
...
func CreateImgPipeline(AccessKey string, SecretKey string, projectName string, infraARN string) (*ec2.CreatePipelineResponse, error) {
	c, err := ec2.NewEC2(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
		Endpoint:   "codepipeline.us-east-1.amazonaws.com",
		Region: "us-east-1",
   ...
   pipeline, err := c.CreatePipeline(context.Background(), pipelineName, infraARN)
   if err != nil {
	   fmt.Println(err)
	   return nil,err
   }
...
func CreateInfrastructureConfiguration(AccessKey string, SecretKey string, projectName string, instProf string) (*ec2.CreateInfrastructureConfigurationResponse, error) {
	c, err := ec2.NewEC2(aws.Config{
		...
		SecretAccessKey: SecretKey,
		SessionToken: "",
		Endpoint:   "imagebuilder.us-east-1.amazonaws.com",
		Region: "us-east-1",
...
func CreateContainerRecipe(AccessKey string, SecretKey string, workdir string, name string, components []models.ImageComponent ,targetRepo ec2.ContainerRepo) (*ec2.CreateContainerRecipeResponse, error) {
	c, err := ec2.NewEC2(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
...
func CreateImageComponents(AccessKey string, SecretKey string, workdir string, name string,s3Info S3DeploymentInfo, service models.Service, files []string) ([]models.ImageComponent, error) {
	c, err := ec2.NewEC2(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
	...
	if err != nil {
		fmt.Println(err)
		return []models.ImageComponent{},err
   }
   ...
   components := []models.ImageComponent{}
   for _, compType := range []string{"base", "dependencies", "app"} {
	   compName := name+"-"+compType
	   compResp, err := c.CreateImageComponent(context.Background(), compName,"1.0.0", "Linux", data)
	   if err != nil {
		   fmt.Println(err)
		   return []models.ImageComponent{},err
	   }
	   component := models.ImageComponent{
		   Name: compResp.Name,
		   Version: compResp.Version,
		   Platform: compResp.Platform,
	   }
