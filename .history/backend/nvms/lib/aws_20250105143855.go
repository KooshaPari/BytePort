...
func PrepareProject(AccessKey string, SecretKey string, project models.Project) (string, string, string, error) {
	zipBall, err := GenerateZip(project)
	if err != nil {
		fmt.Println(err)
		return "","", "", err
	}
	s3Info, err := PushToS3(zipBall, AccessKey, SecretKey, project.Name)
	if err != nil {
		fmt.Println(err)
		return "","", "", err
	}
	return s3Info.BucketName, s3Info.ObjectURL, s3Info.ContentHash, nil
}