...
func PrepareProject(accesskey string, secretkey string, name string, codebase []byte) (*S3DeploymentInfo,*ec2.CreateInstanceProfileResponse,*ecr.CreateRepositoryResponse ,string,string, error) {
	...
	ec2Client, err := ec2.NewEC2(aws.Config{
	 ...
	 subnet1,  sgId,  err := ec2Client.GetInfraNetworkInfo(context.Background() )
	if err != nil {
		fmt.Println(err)
		return nil, nil,nil,"","", err
	}
	return &bucket,instanceProfile,ecrRepo, sgId,subnet1,  nil
	
}
func ProvisionNewNetwork(AccessKey string, SecretKey string, projectName string ) ( *awsnet.CreateLoadBalancerResponse, string, string, error) {
	albClient, err := awsnet.NewALB(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
	...
	ec2Client, err := ec2.NewEC2(aws.Config{
	  ...
	  subnet1,subnet2,  sgId,  err := ec2Client.GetInfraNetworkInfo(context.Background() )
	if err != nil {
		fmt.Println(err)
		return nil,"","", err
	}
	...
	return albInstance, subnet1,sgId,sgId, publicDNS, nil
