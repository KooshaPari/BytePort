...
func CreateImageRecipe(AccessKey string, SecretKey string, workdir string, name string, components []img.ComponentsConfig ,targetRepo img.ContainerRepo) (*img.CreateImageRecipeResponse, error) {
	c, err := img.NewIMG(aws.Config{
		AccessKeyId: AccessKey,
		SecretAccessKey: SecretKey,
		SessionToken: "",
   ...
   recipeName := name+"-byteport-" + uuid.New().String()
   recipe, err := c.CreateImageRecipe(context.Background(), components,recipeName)
   if err != nil {
	   fmt.Println(err)
	   return nil,err
   }
