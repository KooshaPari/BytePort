...
func (c *Client) CreateInfrastructureConfiguration(ctx context.Context, name string, instProfARN string) (*CreateInfrastructureConfigurationResponse, error){
	params := map[string]string{
		"clientToken": time.Now().String(),
		"instanceProfileName": instProfARN,
		"name": name,
	}
	req, err := c.newRequest(ctx, "PUT", params, nil)
	if err != nil {
