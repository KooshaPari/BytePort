...
func (c *Client) CreateContainerRecipe(ctx context.Context,components []models.ImageComponent, name string, targetRepo string, workDir string)(CreateContainerRecipeResponse, error){
	request := CreateContainerRecipeRequest{
		Components: components,
		ClientToken: time.Now().String(),
		SemanticVersion: "1.0.0",
		ContainerType: "DOCKER",
		TargetRepository: { RepositoryName: targetRepo},
		Name: name,
		ParentImage: "ami-0d498bdc7202f4aa6",
