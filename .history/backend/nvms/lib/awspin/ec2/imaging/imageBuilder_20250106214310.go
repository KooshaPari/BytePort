...
func (c *Client) CreatePipeline(ctx context.Context, name string, infraArn string, recipeArn string) (*CreatePipelineResponse, error) {
	...
	reqbody := CreatePipelineRequest{
		...
		Schedule: struct{PipelineExecutionStartCondition string "json:\"pipelineExecutionStartCondition,omitempty\""; ScheduleExpression string "json:\"scheduleExpression,omitempty\""; Timezone string "json:\"timezone,omitempty\""}{},
	}
...
func (c *Client) CreateInfrastructureConfiguration(ctx context.Context, name string, instProfName string, sgIds []string, subId string) (*CreateInfrastructureConfigurationResponse, error){
	// append /CreateInfrastructureConfiguration to the URL
	fmt.Println("Creating Infrastructure Configuration")
	fmt.Println("Profile: '", instProfName,"'")
...
func (c *Client) CreateImageRecipe(ctx context.Context, components []ComponentsConfig, name string) (*CreateImageRecipeResponse, error){
...
"Action": "DeleteImageRecipe",
	...
	req, err := c.newRequest(ctx, "DELETE", params,nil,"/DeleteImageRecipe")
	if err != nil {
		return   err
	}
	resp, err := c.do(req)
...
func (c *Client) CreateImageComponent(ctx context.Context, name string, version string, platform string, data string) (*CreateImageComponentResponse, error){
	fmt.Println("Creating Image Component...")
	request := CreateImageComponentRequest{
		...
		SemanticVersion: "1.0.0",
		Data: data,
		Description: "Image Component for Image Builder",
...
"Action": "CreateComponent",}
	...
	req, err := c.newRequest(ctx, "PUT", params,body,"/CreateComponent")
	if err != nil {
		return nil, err
	}
	resp, err := c.do(req)
	...
	fmt.Println("Image Component Created")
	return &result, nil
...
"Action": "CreateContainerRecipe",
	...
	req, err := c.newRequest(ctx, "PUT", params,body,"/CreateContainerRecipe")
	if err != nil {
		return CreateContainerRecipeResponse{}, err
	}
	resp, err := c.do(req)
