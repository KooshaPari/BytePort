...
func(c *Client) GetInfraNetworkInfo(ctx context.Context )(string, string,error){
    fmt.Println("Getting VPC")
    vpcId,err := c.describeDefaultVPC(ctx)
    if err !=nil {
        return "", "", "","", err
    ...
    //fmt.Println("Security Groups: ", securityGroups)
    //fmt.Println("VPC: ", vpcId)
    subnet1 := subnets.SubnetSet[0].SubnetId 
    return subnet1,  securityGroups.SecurityGroupInfo.Item.GroupId, nil
} 