...
func (c *Client) WaitForIAMRunning(instProfName string, ctx context.Context) error {
    maxAttempts := 60 // Adjust as needed
    fmt.Println("Waiting for Instance Profile to initialize:", instProfName)

    for attempt := 0; attempt < maxAttempts; attempt++ {
        fmt.Printf("Attempt %d: Checking IAM I-Prof statuses\n", attempt+1)
        resp, err := c.GetInstanceProfile(ctx, instProfName)
        if err != nil {
            fmt.Println("Error getting instance profile: ", err)
            // Check if error is InvalidInstanceID.NotFound and retry
            if awsErr, ok := err.(*aws.ErrorResponse); ok && awsErr.Code == "InvalidInstanceID.NotFound" {
                fmt.Println("Instance not yet available, retrying...")
            } else {
            ...
            allRunning := true
            if allRunning && resp.GetInstanceProfileResult.InstanceProfile.InstanceProfileName == instProfName {
                fmt.Println("Instance Profile initialized: ", instProfName)
                return nil
            }
        }
