...
func convertBuildpackToComponent(buildpack models.BuildPack, service models.Service, s3Info S3DeploymentInfo) *models.ImageComponent {
	...
	// Convert buildpack.EnvVars into CLI-friendly exports (or do this in your tool-specific way)
	...
	return &models.ImageComponent{
		Name:        service.Name,
		Version:     "1.0",
		Description: fmt.Sprintf("Buildpack for %s service", service.Name),
		Phases: []models.Phase{
			{
				Name:   "Provision",
				Action: "S3Download",
				Inputs: models.PhaseInputs{
					Source:      s3URI,
					Destination: "/app",
					Overwrite:   true,
				},
			},
			{
				...
				Action: "ExecuteBash",
				Inputs: models.PhaseInputs{
					// Here you can decide to transform `buildpack.Packages`
					// from something like ["golang", "npm"] into
					// dnf/apt install commands as needed.
					// For now, we assume these are already the shell commands:
					Commands: []string{
			...
			},
			{
				...
				Inputs: models.PhaseInputs{
					...
					Commands: []string{buildpack.Start},
				},
			},
		},
	}
...
func convertBuildpackToComponent(buildpack models.BuildPack, service models.Service, s3Info S3DeploymentInfo) *models.ImageComponent {
    ...
    extractionDir := "unzipped-file" // Replace with actual extraction directory logic if needed
                ...
                Steps: []models.Step{
                    {
                        ...
                        Action: "ExecuteBash",
                        Inputs: map[string]interface{}{
                            "commands": append([]string{fmt.Sprintf("cd /app/%s/%s", extractionDir, service.Path)}, buildpack.PreBuild...),
                        },
                    },
                },
            },
                ...
                Steps: []models.Step{
                    {
                        ...
                        Action: "ExecuteBash",
                        Inputs: map[string]interface{}{
                            "commands": append([]string{fmt.Sprintf("cd /app/%s/%s", extractionDir, service.Path)}, buildpack.Build...),
                        },
                    },
                },
            },
                ...
                Steps: []models.Step{
                    {
                        ...
                        Action: "ExecuteBash",
                        Inputs: map[string]interface{}{
                            "commands": []string{fmt.Sprintf("cd /app/%s/%s && %s", extractionDir, service.Path, buildpack.Start)},
                        },
                    },
                },
            },
...
func GenerateImageComponentData(s3Info S3DeploymentInfo, service models.Service, accessKey, secretKey string, files []string) (*models.ImageComponent, error) {
	...
	return convertBuildpackToComponent(*buildpack, service), nil
}