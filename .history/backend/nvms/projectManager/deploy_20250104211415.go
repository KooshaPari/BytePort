...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	if err != nil {
		...
		infraConfig, err := lib.CreateInfrastructureConfiguration(accesskey, secretkey, project.Name, instanceProfile.InstanceProfile.InstanceProfileName)
		for _, service := range nvmsConfig.Services {
		fmt.Println("Serve")
		instances, err := DeployNVMSServiceMVM(accesskey, secretkey, bucket, service,files)
		if err != nil {
