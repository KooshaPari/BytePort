...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	if err != nil {
		...
		ECRinfo := ContainerRepo{
			RepositoryName: ecrRepo.Repository.RepositoryName,
			RepositoryUri:  ecrRepo.Repository.RepositoryUri,
			RepositoryArn:  ecrRepo.Repository.RepositoryArn,
		}
		for _, service := range nvmsConfig.Services {
		fmt.Println("Serve")
		  err := DeployNVMSServiceMVM(accesskey, secretkey, bucket, service,files, project.Name, *instanceProfile, *ecrRepo)
		if err != nil {
