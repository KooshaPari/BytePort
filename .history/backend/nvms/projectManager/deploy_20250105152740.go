...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	bucket, instanceProfile,  ecrRepo, err := lib.PrepareProject(accesskey, secretkey, project.Name, codebase)
  	instance := project.GetDeploy(deployID)
		...
		sgId, subnetID, err := lib.CreateNetwork(accesskey, secretkey, project.Name)
		for _, service := range nvmsConfig.Services {
		fmt.Println("Serve")
		  err := DeployNVMSServiceMVM(accesskey, secretkey, *bucket, service,files, project.Name, *instanceProfile,  ECRinfo, []string{sgId}, subnetId)
		if err != nil {
