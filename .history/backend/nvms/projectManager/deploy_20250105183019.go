...
func DeployProject(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	bucket, instanceProfile,  ecrRepo,sgId, subId,accessURL, err := lib.PrepareProject(accesskey, secretkey, project.Name, codebase)
	if err != nil {
		fmt.Println("Error preparing project: ", err)
		http.Error(w, "Error preparing project: "+err.Error(), http.StatusInternalServerError)
		return}
	err := lib.AwaitIAMInitialization(accesskey, secretkey, instanceProfile.CreateInstanceProfileResult.InstanceProfile.InstanceProfileName)
	if err != nil {
		fmt.Println("Error awaiting IAM initialization: ", err)
		http.Error(w, "Error awaiting IAM initialization: "+err.Error(), http.StatusInternalServerError)
		return
	}
  	...
  	instance := project.GetDeploy(deployID)
		...
		instance.Resources = append(instance.Resources, models.AWSResource{
			...
			ARN:     instanceProfile.CreateInstanceProfileResult.InstanceProfile.Arn,
			Status:  "deployed",
			Region:  instanceProfile.CreateInstanceProfileResult.InstanceProfile.InstanceProfileName,
			ID:      instanceProfile.CreateInstanceProfileResult.InstanceProfile.Arn,
		...
		fmt.Println("Serve")
		  err := DeployNVMSServiceMVM(accesskey, secretkey, *bucket, service,files, project.Name, *instanceProfile,  ECRinfo, []string{sgId}, subId)
		if err != nil {
