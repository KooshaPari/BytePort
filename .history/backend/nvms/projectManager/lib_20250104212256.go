...
func DeployNVMSServiceMVM(AccessKey string, SecretKey string, Bucket lib.S3DeploymentInfo, service models.Service, fileMap []string, projName string, instProf ec2.CreateInstanceProfileResponse ) ( error) {
	...
	containerRecipe, err := lib.CreateContainerRecipe(AccessKey, SecretKey, projName, service, components, infraConfig)
	pipeline, err := lib.CreateImgPipeline(AccessKey, SecretKey, Bucket, service, fileMap, infraConfig)
