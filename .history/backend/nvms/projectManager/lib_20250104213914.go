...
func DeployNVMSServiceMVM(AccessKey string, SecretKey string, Bucket lib.S3DeploymentInfo, service models.Service, fileMap []string, projName string, instProf ec2.CreateInstanceProfileResponse, targetRepo ec2.ContainerRepo) ( error) {
	...
	components, err := lib.CreateImageComponents(AccessKey, SecretKey, "/app"+service.Path, projName+"-"+service.Name)
