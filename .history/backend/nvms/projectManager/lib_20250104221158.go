...
func DeployNVMSServiceMVM(AccessKey string, SecretKey string, s3Info lib.S3DeploymentInfo, service models.Service, fileMap []string, projName string, instProf ec2.CreateInstanceProfileResponse, targetRepo ec2.ContainerRepo, s3Info S3DeploymentInfo) ( error) {
	...
	execPipeline, err := lib.ExecuteImgPipeline(AccessKey, SecretKey, projName, pipeline.ImagePipelineArn)
	if err != nil {
