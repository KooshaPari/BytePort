...
func DeployNVMSServiceMVM(AccessKey string, SecretKey string, s3Info lib.S3DeploymentInfo, service models.Service, fileMap []string, projName string, instProf ec2.CreateInstanceProfileResponse, targetRepo img.ContainerRepo, sgIds []string, subnetId string ) ( error) {
	...
	imageRecipe, err := lib.CreateImageRecipe(AccessKey, SecretKey, projName+"-"+service.Name, components, targetRepo)
	pipeline, err := lib.CreateImgPipeline(AccessKey, SecretKey, projName, infraConfig.InfrastructureConfigurationArn, containerRecipe.ContainerRecipeArn)
	if err != nil {
