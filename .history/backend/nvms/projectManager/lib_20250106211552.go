...
func DeployNVMSServiceMVM(AccessKey string, SecretKey string, s3Info lib.S3DeploymentInfo, service models.Service, fileMap []string, projName string, instProf ec2.CreateInstanceProfileResponse, targetRepo img.ContainerRepo, sgIds []string, subnetId string ) ( error) {
	infraConfig, err := lib.CreateInfrastructureConfiguration(AccessKey, SecretKey, projName, instProf.CreateInstanceProfileResult.InstanceProfile.InstanceProfileName, sgIds, subnetId )
	if err != nil {
	...
	imageRecipe, err := lib.CreateImageRecipe(AccessKey, SecretKey, "/app"+service.Path, projName+"-"+service.Name, components, targetRepo)
	pipeline, err := lib.CreateImgPipeline(AccessKey, SecretKey, projName, infraConfig.InfrastructureConfigurationArn, imageRecipe.ImageRecipeArn)
	if err != nil {
		fmt.Println("Error creating image pipeline: ", err)
		return err;
	}
	execPipeline, err := lib.ExecuteImgPipeline(AccessKey, SecretKey,  pipeline.ImagePipelineArn)
	if err != nil {
		fmt.Println("Error using image pipeline: ", err)
		return err;
	}
	_, err = lib.PushImagetoECR(AccessKey, SecretKey, execPipeline.ImageBuildVersionArn, projName+"-"+service.Name)
	if err != nil {
		fmt.Println("Error pushing image to ECR: ", err)
		return err;
	}
	fmt.Println("Deployed Service: ", service)
	return nil
}