...
// CompositionMetadata identifies the immutable composition handoff for a deployment.
...
// SetCompositionMetadata records the composition render handoff metadata.
func (d *Deployment) SetCompositionMetadata(metadata CompositionMetadata) {
	if d.metadata == nil {
		d.metadata = make(map[string]interface{})
	}
	d.metadata["composition"] = metadata
	d.updatedAt = time.Now().UTC()
}

// CompositionMetadata returns the composition render handoff metadata, if present.
func (d *Deployment) CompositionMetadata() *CompositionMetadata {
	value, ok := d.metadata["composition"].(CompositionMetadata)
	if !ok {
		return nil
	}
	return &value
