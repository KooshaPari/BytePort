...
pub fn parse_config(yaml: &str) -> Result<NVMS, NVMSError> {
    /*
    ...
    let parseFile = match Config::from_str(yaml) {
        Ok(config) => config,
        Err(e) => return Err(NVMSError::YamlError(e.to_string())),
    };
    println!("{:?}", parseFile);
    return parsedConfig;
}
    Ok(parsedConfig)
    Err(NVMSError::YamlError("Error Parsing YAML".to_string()))
}
