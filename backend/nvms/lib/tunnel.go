...
func (tm *TunnelManager) StartProjectTunnel(projectName string) (string, error) {
	...
	cmd := exec.Command("cloudflared", "tunnel", "--config", configFile, "run")
...
func (tm *TunnelManager) StopProjectTunnel(projectName string) error {
	...
	// Kill the process
	err := tunnel.Process.Kill()
	if err != nil {
		return fmt.Errorf("failed to stop tunnel: %w", err)
	}

	// Wait for process to exit
	_, err = tunnel.Process.Wait()
	if err != nil {
		return fmt.Errorf("error waiting for tunnel process to exit: %w", err)
	}
...
func (tm *TunnelManager) GetTunnelStatus(projectName string) string {
	...
	if tunnel, exists := tm.runningTunnels[projectName]; exists {
		...
		if tm.isProcessRunning(tunnel.Process) {
			return "running"
		} else {
...
func (tm *TunnelManager) isProcessRunning(process *os.Process) bool {
	// On Windows, we can check if the process is still running
