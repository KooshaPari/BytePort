<script lang="ts">
   ...
   */
  import { locale, LOCALES, LOCALE_LABELS, LOCALE_FLAGS, setLocale, type Locale } from '$lib/i18n';
      ...
      {#each LOCALES as l (l)}
        <li>
          <button
            ...
            class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-base-200 {l === $locale ? 'font-semibold' : ''}"
            onclick={() => pick(l)}
          >
            <span aria-hidden="true">{LOCALE_FLAGS[l]}</span>
            <span>{LOCALE_LABELS[l]}</span>
            {#if l === $locale}
              <svg class="ml-auto h-4 w-4 text-primary" viewBox="0 0 16 16" fill="none" aria-hidden="true">
