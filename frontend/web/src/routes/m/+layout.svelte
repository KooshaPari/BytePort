  ...
  <aside class="rail" aria-label="Primary">
    ...
    <nav>
      {#each items as it (it.href)}
        <a class="rail-link" class:active={isActive(it.href, $page.url.pathname)} href={it.href}>
...
<style>
  ...
  .bn-item.active { color: var(--primary, #80d5cf); }
  .bn-ico {
    width: 22px;
    height: 22px;
  ...
  .rail nav { display: flex; flex-direction: column; gap: 4px; padding: 0.5rem; }
  .rail-link {
    display: flex;
    align-items: center;
    gap: 0.75rem;
  ...
  .rail-link:hover { background: var(--muted, #101418); color: var(--foreground, #dde4e2); }
  .rail-link.active { background: var(--primary-container, #00504c); color: var(--on-primary-container, #9df1eb); }
  ...
  @media (min-width: 640px) {
    ...
    .content { max-width: calc(100% - 220px); padding: 1.5rem 2rem; }
