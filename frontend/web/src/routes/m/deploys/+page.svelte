<script lang="ts">
  ...
  let deploys = $state<Deploy[]>([
    { id: 'd-1041', project: 'byteport-web',   branch: 'main',         status: 'running',   startedAt: Date.now() - 60_000,  durationSec: 60 },
    { id: 'd-1040', project: 'api-gateway',    branch: 'fix/auth',     status: 'building',  startedAt: Date.now() - 90_000,  durationSec: 90 },
    { id: 'd-1039', project: 'byteport-web',   branch: 'feat/l43-mob', status: 'failed',    startedAt: Date.now() - 1_800_000, durationSec: 12 },
    { id: 'd-1038', project: 'docs-site',      branch: 'main',         status: 'stopped',   startedAt: Date.now() - 7_200_000, durationSec: 240 },
    { id: 'd-1037', project: 'api-gateway',    branch: 'main',         status: 'deploying', startedAt: Date.now() - 30_000,  durationSec: 30 }
  ]);
  ...
  // ── Swipe-to-action ───────────────────────────────────────────────────────
  function swipeOffset(id: string): number { return swipes[id] ?? 0; }
...
<style>
  ...
  .ph { display: flex; align-items: baseline; gap: 0.5rem; }
  .ph h1 { font-size: 1.25rem; margin: 0; }
  .count {
  ...
  .row { position: relative; margin-bottom: 8px; }
  .swipe-bg {
    position: absolute;
    inset: 0;
    display: flex;
  ...
  .swipe-bg.delete { background: var(--destructive, #ffb4ab); color: #690005; justify-content: flex-end; }
  .swipe-bg.redeploy { background: var(--primary, #80d5cf); color: #003734; }
  .card {
    position: relative;
  ...
  .proj { font-weight: 600; color: var(--foreground, #dde4e2); }
  .sub { display: flex; justify-content: space-between; align-items: center; margin-top: 4px; font-size: 0.85rem; color: var(--muted-foreground, #bec9c7); }
  .sub code { background: var(--muted, #101418); padding: 1px 6px; border-radius: 6px; }
  ...
  .pill-running   { background: rgba(128,213,207,0.15); color: #80d5cf; }
  .pill-running .dot { background: #80d5cf; }
  .pill-building  { background: rgba(131,210,227,0.15); color: #83d2e3; }
  .pill-building .dot { background: #83d2e3; animation: blink 1s infinite; }
  .pill-deploying { background: rgba(155,203,251,0.15); color: #9bcbfb; }
  .pill-deploying .dot { background: #9bcbfb; animation: blink 1s infinite; }
  .pill-stopped   { background: rgba(190,201,199,0.12); color: #bec9c7; }
  .pill-stopped .dot { background: #bec9c7; }
  .pill-failed    { background: rgba(255,180,171,0.18); color: #ffb4ab; }
  .pill-failed .dot { background: #ffb4ab; }
