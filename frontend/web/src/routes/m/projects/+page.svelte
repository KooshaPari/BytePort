<script lang="ts">
  ...
  let projects = $state<Project[]>([
    { id: 'p-1', name: 'byteport-web',    framework: 'sveltekit', deployCount: 142, lastDeployedAt: Date.now() - 60_000 },
    { id: 'p-2', name: 'api-gateway',     framework: 'go',        deployCount:  87, lastDeployedAt: Date.now() - 7_200_000 },
    { id: 'p-3', name: 'docs-site',       framework: 'sveltekit', deployCount:  34, lastDeployedAt: Date.now() - 86_400_000 },
    { id: 'p-4', name: 'billing-svc',     framework: 'rust',      deployCount:  19, lastDeployedAt: Date.now() - 604_800_000 }
  ]);
        ...
        <div class="flex items-start justify-between gap-2">
          <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-2 mt-1">
              <span class="badge {frameworkColor(p.framework)} badge-sm">{p.framework}</span>
              <span class="text-xs opacity-70">{p.deployCount} {$t('projects.title').toLowerCase()}</span>
            </div>
          </div>
          <time class="text-xs opacity-70 shrink-0" datetime={new Date(p.lastDeployedAt).toISOString()}>
