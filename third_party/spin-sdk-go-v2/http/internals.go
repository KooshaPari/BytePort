...
func spin_http_handle_http_request(req *C.spin_http_request_t, res *C.spin_http_response_t) {
	var body []byte
	if req.body.is_some {
...
func toSpinHeaders(hm http.Header) C.spin_http_headers_t {
	...
	if headersLen > 0 {
		reqHeaders.len = C.size_t(headersLen)
...
func toSpinBody(body io.Reader) (C.spin_http_option_body_t, error) {
	...
	if len > 0 {
		...
		copy(unsafe.Slice((*byte)(unsafe.Pointer(bodyPtr)), len), buf.Bytes())
